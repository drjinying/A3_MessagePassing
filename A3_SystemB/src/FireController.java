...
class FireController {
	public static void main(String args[]) {
		...
		if (em != null) {
			...
			Indicator fi = new Indicator("Fire Alarm", mw.GetX() + mw.Width(), 0);
			...
			while (!Done) {
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Msg.GetMessageId() == 11) {
						...
						{
							FireAlarmState = true;
							mw.WriteMessage("Received turn on fire alarm message");
						} // if
						
						if (Msg.GetMessage().equalsIgnoreCase("F0")) // turn off fire alarm
						{
							FireAlarmState = false;
							mw.WriteMessage("Received turn off fire alarm message");
						} // if
			...
			if (FireAlarmState)
				fi.SetLampColorAndMessage("Fire Alarm Active", 3);
			else
				fi.SetLampColorAndMessage("Fire Alarm Inactive", 1);
