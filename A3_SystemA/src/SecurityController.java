...
class SecurityController
{
	public static void main(String args[])
	{
		...
		boolean doorBreakAlarmState = false;		// Heater state: false == off, true == on
		boolean windowBreakAlarmState = false;		// Chiller state: false == off, true == on
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					if ( Msg.GetMessageId() == 5 )
					{
						if (Msg.GetMessage().equalsIgnoreCase("db1")) // heater on
						{
							...
							ConfirmMessage( em, "db1" );

						} // if

						if (Msg.GetMessage().equalsIgnoreCase("db0")) // heater off
						{
							...
							ConfirmMessage( em, "db0" );

						} // if

						if (Msg.GetMessage().equalsIgnoreCase("wb1")) // chiller on
						{
							windowBreakAlarmState = true;
							mw.WriteMessage("Received turn on window alarm message" );
							// Confirm that the message was recieved and acted on
							ConfirmMessage( em, "wb1" );

						} // if

						if (Msg.GetMessage().equalsIgnoreCase("wb0")) // chiller off
						{
							...
							ConfirmMessage( em, "wb0" );

						} // if
						if (Msg.GetMessage().equalsIgnoreCase("m1")) // chiller on
						{
						...
						if (Msg.GetMessage().equalsIgnoreCase("m0")) // chiller off
						{
